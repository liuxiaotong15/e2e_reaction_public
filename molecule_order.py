...
if __name__ == '__main__':
    for row in rows:
        ...
        g1 = get_data_pp(idx=row.id, type=G)[0]
        if(atom_cnt_dict.__contains__(k)):
            atom_cnt_dict[k].append((g1, row.id))
        else:
            atom_cnt_dict[k] = [(g1, row.id)]
    print(len(rows)) # 133885
    print(len(atom_cnt_lst)) # 133885
    print(len(atom_cnt_dict)) # 621

    # id, number in group, group total count
    qm9_gibbs_order = []
    for k in list(atom_cnt_dict.keys()):
        # print(sorted(atom_cnt_dict[k]))
        for group_idx, (g, row_idx) in enumerate(sorted(atom_cnt_dict[k])):
            print(rows[row_idx-1], g, group_idx, len(atom_cnt_dict[k]))
            qm9_gibbs_order.append((row_idx, group_idx, len(atom_cnt_dict[k])))

    import pickle
    with open('qm9_gibbs_order.txt', 'wb') as fp:
        pickle.dump(sorted(qm9_gibbs_order), fp)
